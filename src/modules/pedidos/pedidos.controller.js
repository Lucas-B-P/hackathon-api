...
export async function create(request, response, next) {
  try {
    response
      .status(201)
      .json(await service.createOrder(request.user.sub, request.body))
  } catch (error) {
    next(error)
  }
}
export async function list(request, response, next) {
  try {
    response.json({ data: await service.listOrders(request.user.sub) })
  } catch (error) {
    next(error)
  }
}
...
export async function cancel(request, response, next) {
  try {
    await service.cancelOrder(request.user.sub, request.params.id)
