...
export async function getProfile(request, response, next) {
  try {
    response.json(await service.getProfile(request.user.sub))
  } catch (error) {
    next(error)
  }
}
export async function updateProfile(request, response, next) {
  try {
    response.json(await service.updateProfile(request.user.sub, request.body))
  } catch (error) {
    next(error)
  }
}
...
export async function createAddress(request, response, next) {
  try {
    response
      .status(201)
      .json(await service.createAddress(request.user.sub, request.body))
  } catch (error) {
    next(error)
  }
}
...
export async function updatePreferences(request, response, next) {
  try {
    response.json(
      await service.updatePreferences(request.user.sub, request.body),
    )
  } catch (error) {
    next(error)
  }
  ...
  try {
    ...
    if (!senhaAtual || !novaSenha || novaSenha.length < 6)
      return response.status(400).json({
        error: { code: "VALIDATION_ERROR", message: "Senhas inválidas" },
      })
