...
export async function updateProfile(userId, data) {
  const allowed = { nome: "name", cpf: "cpf", telefone: "phone", nascimento: "birth_date", fotoUrl: "avatar_url" };
  const entries = Object.entries(data).filter(([key, value]) => allowed[key] && value !== undefined);
...
export async function getProfile(userId) {
  const result = await pool.query("SELECT id, name, email, cpf, phone, birth_date, avatar_url, role FROM users WHERE id = $1", [userId]);
  return result.rows[0];
}

export async function listAddresses(userId) {
  const result = await pool.query("SELECT id, label, street, number, complement, neighborhood, city, state, zip_code, is_primary FROM user_addresses WHERE user_id = $1 ORDER BY is_primary DESC, id", [userId]);
  return result.rows;
}
...
export async function getPreferences(userId) {
  const result = await pool.query("SELECT email_notifications, sms_notifications, appointment_reminders, marketing_notifications FROM user_preferences WHERE user_id = $1", [userId]);
  if (result.rows[0]) return result.rows[0];
  const created = await pool.query("INSERT INTO user_preferences (user_id) VALUES ($1) RETURNING email_notifications, sms_notifications, appointment_reminders, marketing_notifications", [userId]);
  return created.rows[0];
}

export async function updatePreferences(userId, data) {
  await getPreferences(userId);
  const allowed = { emailNotifications: "email_notifications", smsNotifications: "sms_notifications", appointmentReminders: "appointment_reminders", marketingNotifications: "marketing_notifications" };
...
export async function changePassword(userId, currentPassword, newPassword) {
  ...
  if (!result.rows[0] || !(await bcrypt.compare(currentPassword, result.rows[0].password_hash))) {
    const error = new Error("Senha atual inválida"); error.statusCode = 400; throw error;
  }
