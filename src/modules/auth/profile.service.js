...
export async function updateProfile(userId, data) {
  ...
  values.push(userId)
  await pool.query(
    `UPDATE users SET ${set}, updated_at = NOW() WHERE id = $${values.length}`,
    values,
  )
...
export async function updatePreferences(userId, data) {
  ...
  const entries = Object.entries(data).filter(
    ([key, value]) => allowed[key] && typeof value === "boolean",
  )
  if (entries.length) {
    const values = entries.map(([, value]) => value)
    values.push(userId)
    const set = entries
      ...
      .join(", ")
    await pool.query(
      `UPDATE user_preferences SET ${set}, updated_at = NOW() WHERE user_id = $${values.length}`,
      values,
    )
  }
