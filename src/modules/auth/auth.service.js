...
export async function requestPasswordReset(email) {
  ...
  await pool.query(
    "INSERT INTO password_reset_tokens (user_id, token_hash, expires_at) VALUES ($1, $2, datetime('now', '+30 minutes'))",
    [user.id, tokenHash],
  )

  if (!env.smtp.host || !env.smtp.user || !env.smtp.password) {
    console.warn(
      `SMTP não configurado. Token de recuperação para ${user.email}: ${rawToken}`,
    )
    return
  }
  ...
  await transporter.sendMail({
    from: env.smtp.from,
    to: user.email,
    subject: "Redefina sua senha — Patinhas Pet Shop",
    text: `Olá, ${user.name}! Acesse ${resetUrl} para criar uma nova senha. O link expira em 30 minutos.`,
  })
}
